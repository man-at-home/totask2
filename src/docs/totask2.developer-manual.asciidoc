...
* git installieren                     http://git-scm.com
...
Run mit InMemory Datenbank => `%> gradle bootRun`

Run mit persistenter Datenbank => `%> gradle bootRun -Dspring.profiles.active=qa`
...
----
<1> already deployed scripts
<2> scripts needing deployment
<3> deploy scripts to database via command line
