...
checkstyle, pmd, findbugs, asciidoc. Ein Build-Server http://jenkins-ci.org/[jenkins] läuft in der cloud (openshift).

Experimentell ist http://www.seleniumhq.org/[selenium] als Web Test Tool.

Als "echter" Appserver für das Deployment fungiert der http://www.wildfly.org/[WildFly Application Server] von redhat.
Hosting der Anwendung in der cloud erfolgt unter openShift in einer https://developers.openshift.com/en/wildfly-overview.html[WildFly Cardridge].
...
image::images/totask2.cloud.jenkins.png[screenshot_CLOUD_1, 600, 400]
...
* gradle tests laufen derzeit nicht auf dem jenkins cloud server (incompatibilität gradle 2 und openShift?)
* bekomme asciidoctor-diagram im gradle build nicht ans laufen, daher mit Umweg (umkopieren der generierten Bilder aus JavaDoc Erzeugung)
