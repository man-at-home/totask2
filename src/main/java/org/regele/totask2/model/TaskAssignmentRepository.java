...
public interface TaskAssignmentRepository extends JpaRepository<TaskAssignment, Long>  {
    ...
    @Query("select ta from TaskAssignment ta where ta.user.id = :userId and ta.startingFrom <= :until or ta.until >= :from")    
    List<TaskAssignment> findByUserAndPeriod(@Param("userId") long userId, @Param("from") Date from, @Param("until") Date until);
