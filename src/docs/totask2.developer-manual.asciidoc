...
Compile und Test des Projekts erfolgt mit => `%> gradle test`.

Dokumentation erstellen (einschließlich diesem Dokument hier) => `%> gradle allDocs`.

Run mit InMemory Datenbank h2 => `%> gradle bootRun`

Run mit persistenter Datenbank (die Daten bleiben zwischen den Runs erhalten) => `%> gradle bootRun -Dspring.profiles.active=qa`
...
** `git push cloud`
** remote clould: `ssh://5******4@h***e.rhcloud.com/~/git/host.git/`
...
image::images/totask2.cloud.wildfly.deploy.png[screenshot.sftp, 400, 200]


[[img-rhc]]
.rhc command line openShift
image::images/totask2.cloud.rhc.png[screenshot.sftp, 400, 200]
...
aber mit Flyway verwaltet. Dieses kleine Tool verwaltet je Datenbank den aktuellen und führt bei Bedarf beim Start der Anwendung
notwendige Migration am Datenmodell durch.  
...
gradle ist ein Groovy basierendes Build System, als alternative zu maven oder ant zu empfehlen.
